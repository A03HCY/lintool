...
# 设置一个环境变量标志，防止递归调用
if os.environ.get('DLSO_BUILDING') == '1':
    print("检测到嵌套构建调用，退出...")
    sys.exit(1)

os.environ['DLSO_BUILDING'] = '1'
...
# 解析命令行参数
def parse_args():
    parser = argparse.ArgumentParser(description="dlso构建脚本")
...
def get_version():
    try:
        ...
        spec = importlib.util.spec_from_file_location("dlso", "dlso/__init__.py")
...
def clean_build_dirs():
    """清理构建目录"""
    dirs_to_clean = ['build', 'dist', 'dlso.egg-info']
...
def build_package():
    ...
    with Progress(
        ...
        build_task = progress.add_task("[bold cyan]构建dlso包", total=100)
...
def main():
    ...
    # 使用分割线显示标题
    console.rule(f"[bold cyan]dlso 构建脚本 v{version}[/bold cyan]")
    ...
    try:
        ...
        if build_success:
            ...
            if args.install:
                ...
                if install_success:
                    console.print("\n[bold cyan]您可以现在导入dlso库进行测试。[/bold cyan]")
